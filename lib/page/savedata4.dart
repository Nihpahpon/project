...
class Savedata4 extends StatefulWidget {
  const Savedata4({Key? key}) : super(key: key);

  @override
  State<Savedata4> createState() => _Savedata4State();
}

class _Savedata4State extends State<Savedata4> {
  final formKey = GlobalKey<FormState>();
  String? dropValue2, species, disease;
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              items: <String>['โรคใบไหม้แผลใหญ่ ', 'โรคใบจุด ', 'โรคราสนิม ']
                  .map<DropdownMenuItem<String>>((String value) {
                return DropdownMenuItem<String>(
                  value: value,
                  child: Padding(
  ...
  Future<void> creadata4() async {
    final double area = double.parse(_areaController.text);
    await FirebaseFirestore.instance.collection('disease').doc("$disease").set({
      'area': (area),
      'disease': (disease!),
    }).then((value) {
      print('okkkk');
  ...
  Future<void> creadata5() async {
    final double area = double.parse(_areaController.text);
    await FirebaseFirestore.instance.collection('species').doc(species!).set({
      'species': (species!),
      'area': (area),
    }).then((value) {
      print('okkkk');
...
// class _Savedata4State extends State<Savedata4> {
//   final formKey = GlobalKey<FormState>();
//   String? disease, dropValue2, area, species;
//   final TextEditingController _areaController = TextEditingController();
//   late String date;
//   var now = DateTime.now();
//   User? user = FirebaseAuth.instance.currentUser;
...
//             ),
//             onPressed: () {
//               final double? price = double.tryParse(_areaController.text);
//               if (dropValue2 == null) {
//                 Mydialog().normalDialog(
//                     context,
//                     'คุณยังไม่ได้เลือกอายุของต้นข้าวโพด',
...
//   Future<void> creadata1() async {
//     final double? area = double.tryParse(_areaController.text);
//     await FirebaseFirestore.instance.collection('area').doc(id).set({
//       'id': (id),
//       'location': GeoPoint(lat!, lng!),
//       'area': (area!),
//       'species': (species!),
